...
std::vector<std::string> vGenesisMembers;

void ReadGenesisMembers()
{
    fs::path absolute_path=GetConfigFile("genesis_members.conf");
    fs::ifstream fin(absolute_path);
    char line[1024]={0};
    std::string tmp="";
    while(fin.getline(line, sizeof(line)))
    {
        std::stringstream member_vrfpk(line);
        member_vrfpk>>tmp;
        vGenesisMembers.push_back(tmp);
    }
    fin.clear();
    fin.close();
}
